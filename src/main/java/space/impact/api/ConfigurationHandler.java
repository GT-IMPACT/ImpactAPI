...
public enum ConfigurationHandler {
	...
	private void setLanguageKeys() {
		for (String categoryName : config.getCategoryNames()) {
			...
			category.setLanguageKey("impactapi.config." + categoryName);
	...
	private void loadConfig() {
		maxCoexistingHologram   = config.getInt("maxCoexisting", "client.hologram",
				1, 1, 100, "An attempt will be made to prune old holograms when a new hologram is about to be projected");
		removeCollidingHologram = config.getBoolean("removeColliding", "client.hologram",
				true, "An attempt will be made to remove an existing hologram if it collides with a new hologram.");
