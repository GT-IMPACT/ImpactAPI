...
import space.impact.api.multiblocks.structure.adders.IBlockAdder;
import space.impact.api.multiblocks.structure.adders.ITileAdder;
...
/**
 * API для проверки структуры!
 * <p>
 * (Просто импортируйте статически этот класс, чтобы иметь хороший беглый синтаксис при определении определений структуры)
 */
public class StructureUtility {
	...
	private static final IStructureElement AIR = new IStructureElement() {
		...
		public boolean spawnHint(Object o, World world, int x, int y, int z, ItemStack trigger) {
			ImpactAPI.hintParticle(world, x, y, z, ImpactAPI.getBlockHint(), 13);
			return true;
		}
	...
	private static final IStructureElement NOT_AIR = new IStructureElement() {
		...
		@Override
		public boolean spawnHint(Object o, World world, int x, int y, int z, ItemStack trigger) {
			ImpactAPI.hintParticle(world, x, y, z, ImpactAPI.getBlockHint(), 14);
			return true;
		}
		
		@Override
		public boolean placeBlock(Object o, World world, int x, int y, int z, ItemStack trigger) {
			world.setBlock(x, y, z, ImpactAPI.getBlockHint(), 14, 2);
			return true;
		}
	};
	@SuppressWarnings("rawtypes")
	private static final IStructureElement ERROR = new IStructureElement() {
		...
		@Override
		public boolean spawnHint(Object o, World world, int x, int y, int z, ItemStack trigger) {
			ImpactAPI.hintParticle(world, x, y, z, ImpactAPI.getBlockHint(), 15);
			return true;
		}
	 ...
	 * Размещение всегда обрабатывается этим и ничего не делает.
	 * Практически не используется в цепочке fallback.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Проверка всегда возвращает: true.
	 */
	public static <T> IStructureElementNoPlacement<T> ofHint(int dots) {
		...
		return new IStructureElementNoPlacement<T>() {
			...
			public boolean spawnHint(T t, World world, int x, int y, int z, ItemStack trigger) {
				ImpactAPI.hintParticle(world, x, y, z, ImpactAPI.getBlockHint(), meta);
				return false;
			}
		};
	}
	...
	public static <T> IStructureElementNoPlacement<T> ofHintDeferred(Supplier<IIcon[]> icons) {
		return new IStructureElementNoPlacement<T>() {
			...
			@Override
			public boolean spawnHint(T t, World world, int x, int y, int z, ItemStack trigger) {
				ImpactAPI.hintParticle(world, x, y, z, icons.get());
				return false;
			}
		};
	}
	...
	public static <T> IStructureElementNoPlacement<T> ofHintDeferred(Supplier<IIcon[]> icons, short[] RGBa) {
		return new IStructureElementNoPlacement<T>() {
			...
			@Override
			public boolean spawnHint(T t, World world, int x, int y, int z, ItemStack trigger) {
				ImpactAPI.hintParticleTinted(world, x, y, z, icons.get(), RGBa);
				return false;
			}
		};
	}
	...
	/**
	 * Более примитивный вариант {@link #ofBlocksTiered(ITierConverter, List, Object, BiConsumer, Function)}.
	 * Основное отличие в том, что этот вариант только для проверки.
	 *
	...
	/**
	 * Элемент, представляющий компонент с различными уровнями.
	 * Игрок может использовать больше чертежей, чтобы получить подсказки, обозначающие более продвинутые компоненты.
	 * <p>
	 * TODO В этой утилите пока нет контр. части TileEntity.
	 *
	 * @param allKnownTiers - Все известные уровни на момент вызова. Может быть пустым или нулевым.
	 *                      При пустом или нулевом значении подсказка не будет порождена. Не может иметь нулевых элементов.
	 *                      Первый элемент обозначает самый примитивный уровень. Последний элемент обозначает самый продвинутый уровень.
	 *                      Если не все уровни доступны во время построения определения используйте {@link #lazy(Supplier)} или его перегрузки для небольшой задержки.
	 * @param notSet        - Значение, возвращаемое из {@code getter}, когда в T еще не найдена информация об уровне. Может быть равно null.
	 * @param getter        - функция для получения текущего уровня из T
	 * @param setter        - функция для установки текущего уровня в T
	 * @param tierExtractor - функция для извлечения информации о тире из блока.
	 */
	public static <T, TIER> IStructureElement<T> ofBlocksTiered(ITierConverter<TIER> tierExtractor, @Nullable List<Pair<Block, Integer>> allKnownTiers, @Nullable TIER notSet, BiConsumer<T, TIER> setter, Function<T, TIER> getter) {
		...
		return new IStructureElement<T>() {
			...
			public boolean spawnHint(T t, World world, int x, int y, int z, ItemStack trigger) {
				...
				if (hint == null)
					return false;
				ImpactAPI.hintParticle(world, x, y, z, hint.getKey(), hint.getValue());
				return true;
			}
	...
	/**
	 * Обозначить блок, используя нелокализованные имена. Это может быть полезно для обхода проблем с порядком загрузки мода.
	 * <p>
	 * Хотя немедленной ошибки не произойдет, клиентский код должен убедиться, что указанный мод
	 * загружен и что указанный мод присутствует, иначе в дальнейшем произойдут плохие вещи!
	 */
	public static <T> IStructureElement<T> ofBlockUnlocalizedName(String modid, String unlocalizedName, int meta) {
		...
		return new IStructureElement<T>() {
			...
			@Override
			public boolean spawnHint(T t, World world, int x, int y, int z, ItemStack trigger) {
				ImpactAPI.hintParticle(world, x, y, z, getBlock(), meta);
				return true;
			}
	...
	/**
	 * Аналогична другой перегрузке, но позволяет клиентскому коду указать обратный ход
	 * в случае, если указанный блок не был найден впоследствии. при вызове элемента.
	 * <p>
	 * Это несколько отличается от использования другой перегрузки и другого
	 * элемента в {@link #ofChain(IStructureElement[])}, поскольку такая комбинация приведет к ошибке, а эта - нет.
	 */
	public static <T> IStructureElement<T> ofBlockUnlocalizedName(String modid, String unlocalizedName, int meta, IStructureElement<T> fallback) {
		...
		return new IStructureElement<T>() {
			...
			public boolean spawnHint(T t, World world, int x, int y, int z, ItemStack trigger) {
				if (init()) {
					ImpactAPI.hintParticle(world, x, y, z, block, meta);
					return true;
				} else
	...
	/**
	 * Не допускает дублирования блоков (с разными метами)
	 */
	...
	/**
	 * Позволяет дублировать блоки (с разными метами)
	 */
	...
	public static <T> IStructureElement<T> ofBlocksFlat(Map<Block, Integer> blocsMap, Block defaultBlock, int defaultMeta) {
		...
		if (defaultBlock instanceof ICustomBlockSetting) {
			return new IStructureElement<T>() {
				...
				@Override
				public boolean spawnHint(T t, World world, int x, int y, int z, ItemStack trigger) {
					ImpactAPI.hintParticle(world, x, y, z, defaultBlock, defaultMeta);
					return true;
				}
			};
		} else {
	...
	/**
	 * То же, что и выше, но игнорируется мета-идентификатор цели
	 */
	...
	/**
	 * То же, что и выше, но позволяет задать рендеринг частиц с подсказкой
	 */
	...
	public static <T> IStructureElement<T> ofBlockAdder(IBlockAdder<T> iBlockAdder, int dots) {
		return ofBlockAdder(iBlockAdder, ImpactAPI.getBlockHint(), dots - 1);
	}
	...
	// Больше не нужно добавлять штриховку. Реализуйте его через тайловый аддер. Мы, конечно, можем добавить обертку вокруг него в gregtech, но больше не в этом отдельном моде.
	...
	/**
	 * Будьте осторожны при построении цепочки, так как она будет пытаться вызвать каждый элемент структуры, пока не вернет true.
	...
	/**
	 * Аналогично defer, но кэширует первый возвращенный элемент и не будет вызывать его снова.
	 * Инициализация не является потокобезопасной.
	 */
	...
	/**
	 * Используется внутренне, для создания пропусков при определении структуры
	 */
	...
	/**
	 * Используется только для получения псевдокода в программе написания структур...
	 * <p>
	 * NOTE: Код, специфичный для GT, был удален. TODO добавить среднее значение
	 *
	 * @param tileEntityClassifier - строку, обозначающую тип TileEntity или null, если в ней нет ничего особенного.
	 *                             Полезно, если сущность плитки не может быть просто определена с помощью getClass.
	 */
	public static String getPseudoJavaCode(World world, ExtendedFacing extendedFacing, int basePositionX, int basePositionY, int basePositionZ,
										   int basePositionA, int basePositionB, int basePositionC, Function<? super TileEntity, String> tileEntityClassifier,
	...
	/**
	 * Транспонирует фигуру (меняет местами оси B и C, может использоваться для отмены транспонирования транспонированной фигуры)
	 * ВНИМАНИЕ! Не используйте на старых api...
	 *
	 * @param structurePiece форма (транспонированная форма)
	 * @return транспонированная форма (не транспонированная форма)
	 */
