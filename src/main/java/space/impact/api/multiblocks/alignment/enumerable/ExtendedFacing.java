...
import space.impact.api.multiblocks.alignment.IntegerAxisSwap;
...
public enum ExtendedFacing {
	...
	ExtendedFacing(String name) {
		this.name = name;
		direction = Direction.VALUES[ordinal() / (IAlignment.ROTATIONS_COUNT * IAlignment.FLIPS_COUNT)].getForgeDirection();
		rotation  = Rotation.VALUES[ordinal() / IAlignment.FLIPS_COUNT - direction.ordinal() * IAlignment.ROTATIONS_COUNT];
		flip      = Flip.VALUES[ordinal() % IAlignment.FLIPS_COUNT];
	 ...
	 * @return Vec3 - A, B, C offset (facing relative  L--R, U--D, F--B)
	 */
